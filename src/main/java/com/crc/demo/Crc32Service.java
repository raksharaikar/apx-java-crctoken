...
public class Crc32Service {
    ...
    // Method to extract version from the firmware filename (e.g., OTA_20.bin -> version 20)
    private int getVersion(String filename) {
        String versionStr = filename.replaceAll("[^0-9]", ""); // Extract numeric part of filename
        return Integer.parseInt(versionStr);
    }

    // Method to get the latest firmware file based on version comparison
    public File getLatestFirmware() throws IOException {
        File firmwareDirectory = new File(FIRMWARE_DIRECTORY);

        // List all firmware files in the directory
        File[] firmwareFiles = firmwareDirectory.listFiles((dir, name) -> name.endsWith(".bin"));
        if (firmwareFiles == null || firmwareFiles.length == 0) {
            throw new IOException("No firmware files found.");
        }

        // Sort the files by version number extracted from filenames
        List<File> sortedFirmwareFiles = Arrays.stream(firmwareFiles)
                .sorted((f1, f2) -> Integer.compare(getVersion(f2.getName()), getVersion(f1.getName()))) // Sort descending by version
                .collect(Collectors.toList());

        // Return the latest firmware file (the first in the sorted list)
        return sortedFirmwareFiles.get(0);
    }

    // Method to list available firmware versions (file names)
    public List<String> getAvailableFirmwareVersions() {
        File firmwareDirectory = new File(FIRMWARE_DIRECTORY);
        File[] firmwareFiles = firmwareDirectory.listFiles((dir, name) -> name.endsWith(".bin"));
        if (firmwareFiles == null || firmwareFiles.length == 0) {
            return List.of(); // Return empty list if no firmware files are found
        }
        return Arrays.stream(firmwareFiles)
                .map(file -> file.getName()) // Return the file name as the version
                .collect(Collectors.toList());
    }
 


    public static int crc_ta_8[]= new int[]{
